...
def sample_instance_intervals(instances, duration):
    """
    ...
    * duration: Desired duration of sampled intervals(in seconds).\n
    ...
    for instance in instances:
        ...
        if instance_duration < duration:
            ...
            context_before = round(random.uniform(0, context), 3)
            context_after = context - context_before
            start = round(start - context_before, 3)
            end = round(end + context_after, 3)
        elif instance_duration > duration:
            start = round(random.uniform(start, end - duration), 3)
